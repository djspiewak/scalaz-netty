...
private[netty] final class Client(channel: _root_.io.netty.channel.Channel, queue: async.mutable.Queue[ByteVector], limit: Int) {
...
private[netty] object Client {
  def apply(to: InetSocketAddress, config: ClientConfig)(implicit pool: ExecutorService, S: Strategy): Task[Client] = Task delay {
    ...
    for {
      ...
      client <- Task delay {
        new Client(connectF.channel(), queue, config.limit)
      }
    } yield client
  } join
}
